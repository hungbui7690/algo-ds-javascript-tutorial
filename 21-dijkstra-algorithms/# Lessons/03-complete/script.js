...
class WeightedGraph {
  ...
  Dijkstra(start, finish) {
    ...
    // (***) ko phải queue.length, mà là queue.items.length
    while (queue.items.length) {
      // (a) thằng nhỏ nhất trong distances {}
      smallest = queue.dequeue().value // trả về A, B, C
      // (c)
      if (smallest === finish) {
      ...
      if (smallest || distances[smallest] !== Infinity) {
        // (***) this.adjacencyList[smallest] trả về array
        //>> [{node : B, weight : 4}, {node : C, weight: 2}]
        this.adjacencyList[smallest].forEach((neighbor, index) => {
          // (i) giá trị sẽ update vào trong distances {}
          let candidate = distances[smallest] + neighbor.weight
          let neighborNode = neighbor.node
          // (ii) so sánh vào update vào trong distances {}
